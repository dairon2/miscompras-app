...
export default function AdvanceDetailPage() {
    ...
    const downloadPdf = async () => {
        if (!advance) return;
        try { const result = await api.get(`/advances/${advance.id}/pdf`, { responseType: 'blob' }); const url = URL.createObjectURL(result.data); const link = document.createElement('a'); const formatName = advance.beneficiaryType === 'EMPLOYEE' ? 'Empleado' : 'Tercero'; link.href = url; link.download = `Solicitud_Anticipo_${formatName}_${advance.year}_${advance.consecutive}.pdf`; link.click(); URL.revokeObjectURL(url); }
    ...
    return <div className="p-6 lg:p-10 max-w-6xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6"><section className="lg:col-span-2 bg-white dark:bg-gray-800 rounded-2xl border p-6"><h2 className="font-bold flex items-center gap-2"><WalletCards className="w-5 h-5 text-teal-600" />Información del anticipo</h2><div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-5 mt-6 text-sm"><Info label="Fecha" value={new Date(advance.requestDate).toLocaleDateString('es-CO')} /><Info label="Tipo" value={advance.beneficiaryType === 'EMPLOYEE' ? 'Empleado del Museo' : 'Proveedor, contratista o prestador de servicio'} /><Info label="Centro de costos" value={advance.costCenter || '-'} /><Info label="Código centro de costos" value={advance.costCenterCode || '-'} /><Info label="Proyecto" value={advance.project?.name || '-'} /><Info label="Área" value={advance.area?.name || '-'} /><Info label="Presupuesto" value={advance.budget ? `${advance.budget.code || ''} ${advance.budget.title}` : '-'} /><Info label="Requerimiento" value={advance.requirement?.title || '-'} /><div className="sm:col-span-2"><Info label="Objeto" value={advance.purpose} /></div>{advance.legalizationDueDate && <Info label="Fecha máxima de legalización" value={new Date(advance.legalizationDueDate).toLocaleDateString('es-CO')} />}{advance.legalizationNotes && <div className="sm:col-span-2"><Info label="Observaciones de legalización" value={advance.legalizationNotes} /></div>}{advance.cancellationReason && <div className="sm:col-span-2"><Info label="Motivo de anulación" value={advance.cancellationReason} /></div>}</div></section>
