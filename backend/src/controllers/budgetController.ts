...
import { generateBudgetPDF, generateAdjustmentPDF } from '../services/pdfService';
import { sendBudgetNotificationEmail, sendAdjustmentNotificationEmail } from '../services/emailService';
...
export const getBudgets = async (req: AuthRequest, res: Response) => {
    try {
        ...
        // Base where clause
        const where: any = { year: filterYear };
        ...
        const budget = await prisma.budget.create({
            data: {
                ...
                subLeaders: subLeaders?.length > 0 ? {
                    create: subLeaders.map((id: string) => ({ userId: id }))
                } : undefined
            },
            include: {
                project: true,
    ...
    try {
        ...
        if (subLeaders) {
            ...
            if (subLeaders.length > 0) {
                await prisma.budgetSubLeader.createMany({
                    data: subLeaders.map((userId: string) => ({ budgetId: id, userId }))
                });
            }
        }
...
export const deleteBudget = async (req: AuthRequest, res: Response) => {
    try {
        ...
        const reqCount = await prisma.requirement.count({ where: { budgetId: id } });
        if (reqCount > 0) {
...
export const getBudgetYears = async (req: AuthRequest, res: Response) => {
    try {
...
export const createMassBudgets = async (req: AuthRequest, res: Response) => {
    try {
        const userRole = req.user?.role;
        const userId = req.user?.id;
...
export const approveBudgetGroup = async (req: AuthRequest, res: Response) => {
    try {
        const userId = req.user?.id;
        ...
        // Update all
        const updateResult = await prisma.budget.updateMany({
            where: {
...
export const rejectBudgetGroup = async (req: AuthRequest, res: Response) => {
    try {
        const userId = req.user?.id;
        const { id } = req.params; // Group ID
